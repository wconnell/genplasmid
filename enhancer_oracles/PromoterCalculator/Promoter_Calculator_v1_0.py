import util, random, sys, pickle, collections, operator, itertools, time, math, os, argparse
...
def scan_arbitrary(inputSequence, two_mer_encoder, three_mer_encoder, model, inters, constraints, dg10_0, dg10_3,
                   dg35_0, dg35_3, dmers, x10mers, spacers):
    seq_query = {}
        ...
        for j in range(15, 21):
            ...
            for k in range(6, 11):
                ...
                if len(tempITR) < 20:
                    ...
                    dG_total, dG_apparent, dg10, dg35, dg_disc, dg_ITR, dg_ext10, dg_spacer, dg_UP = linear_free_energy_model(
                        tempUP, temp35, tempspacer, temp10, tempdisc, tempITR, dg10_0, dg10_3, dg35_0, dg35_3, dmers,
                        x10mers, spacers, model, inters)
                    ...
                    seq_query[(float(dG_total), float(dG_apparent), TSS_distance)] = (
                        (tempUP, temp35, tempspacer, temp10, tempdisc, tempITR),
                        (dg10, dg35, dg_disc, dg_ITR, dg_ext10, dg_spacer, dg_UP))
...
def linear_free_energy_model(UP, h35, spacer, h10, disc, ITR, dg10_0, dg10_3, dg35_0, dg35_3, dmers, x10mers, spacers,
                             coeffs, inters):
...
def predict(sequence, constraints):
    ...
    dg10_0, dg10_3, dg35_0, dg35_3, dmers, x10mers, spacers = get_matrices(two_mer_encoder=two_mer_encoder,
                                                                           three_mer_encoder=three_mer_encoder,
                                                                           spacer_encoder=spacer_encoder, coeffs=layer1)

    # Scan DNA and return predictions
    (od, result), query = scan_arbitrary(inputSequence=sequence, two_mer_encoder=two_mer_encoder,
                                         three_mer_encoder=three_mer_encoder,
                                         model=layer1, inters=inters, constraints=constraints, dg10_0=dg10_0,
                                         dg10_3=dg10_3,
                                         dg35_0=dg35_0, dg35_3=dg35_3, dmers=dmers, x10mers=x10mers, spacers=spacers)

    dG_total, UP, h35, spacer, h10, disc, ITR = result[0][0], result[1][0][0], result[1][0][1], result[1][0][2], \
        result[1][0][3], result[1][0][4], result[1][0][5]
...
class Promoter_Calculator(object):
    ...
    def __init__(self, organism='Escherichia coli str. K-12 substr. MG1655',
        ...
        self.dg10_0, self.dg10_3, self.dg35_0, self.dg35_3, self.dmers, self.x10mers, self.spacers = get_matrices(
            two_mer_encoder=self.two_mer_encoder, three_mer_encoder=self.three_mer_encoder,
            spacer_encoder=self.spacer_encoder, coeffs=self.layer1)
    ...
    def predict(self, sequence, TSS_range):
        ...
        MinPromoterSize = UPS_length + UPS_HEX35_SPACER + HEX35_length + SPACER_length_range[0] + HEX10_length + \
                          DISC_length_range[0] + ITR_length
        MaxPromoterSize = UPS_length + UPS_HEX35_SPACER + HEX35_length + SPACER_length_range[1] + HEX10_length + \
                          DISC_length_range[1] + ITR_length
        MinimumTSS = UPS_length + UPS_HEX35_SPACER + HEX35_length + SPACER_length_range[0] + HEX10_length + \
                     DISC_length_range[0]
                ...
                if TSS - DISC_length >= 0 and TSS + ITR_length <= len(sequence):
                    ...
                    for SPACER_length in range(SPACER_length_range[0], SPACER_length_range[1]):
                        ...
                        if TSS - DISC_length - HEX10_length - SPACER_length - HEX35_length - UPS_length - UPS_HEX35_SPACER >= 0:
                            ...
                            tempspacer = sequence[
                                         TSS - DISC_length - HEX10_length - SPACER_length: TSS - DISC_length - HEX10_length]
                            temp35 = sequence[
                                     TSS - DISC_length - HEX10_length - SPACER_length - HEX35_length: TSS - DISC_length - HEX10_length - SPACER_length]
                            tempUP = sequence[
                                     TSS - DISC_length - HEX10_length - SPACER_length - HEX35_length - UPS_length - UPS_HEX35_SPACER:  TSS - DISC_length - HEX10_length - SPACER_length - HEX35_length - UPS_HEX35_SPACER]

                            dG_total, dG_apparent, dG_10, dG_35, dG_disc, dG_ITR, dG_ext10, dG_spacer, dG_UP = linear_free_energy_model(
                                tempUP, temp35, tempspacer, temp10, tempdisc, tempITR, self.dg10_0, self.dg10_3,
                                self.dg35_0, self.dg35_3, self.dmers, self.x10mers, self.spacers, self.model,
                                self.inters)
                            dG_bind = dG_10 + dG_35 + dG_spacer + dG_ext10 + dG_UP

                            Tx_rate = self.K * math.exp(- self.BETA * dG_total)

                            result = {'promoter_sequence': sequence[
                                                           TSS - DISC_length - HEX10_length - SPACER_length - HEX35_length - UPS_length - UPS_HEX35_SPACER: TSS + ITR_length],
                                      'TSS': TSS, 'UP': tempUP, 'hex35': temp35, 'spacer': tempspacer, 'hex10': temp10,
                                      'disc': tempdisc, 'ITR': tempITR,
                                      'dG_total': dG_total, 'dG_10': dG_10, 'dG_35': dG_35, 'dG_disc': dG_disc,
                                      'dG_ITR': dG_ITR, 'dG_ext10': dG_ext10, 'dG_spacer': dG_spacer, 'dG_UP': dG_UP,
                                      'dG_bind': dG_bind,
                                      'Tx_rate': Tx_rate,
                                      'UP_position': [
                                          TSS - DISC_length - HEX10_length - SPACER_length - HEX35_length - UPS_HEX35_SPACER - UPS_length,
                                          TSS - DISC_length - HEX10_length - SPACER_length - HEX35_length - UPS_HEX35_SPACER],
                                      'hex35_position': [
                                          TSS - DISC_length - HEX10_length - SPACER_length - HEX35_length,
                                          TSS - DISC_length - HEX10_length - SPACER_length],
                                      'spacer_position': [TSS - DISC_length - HEX10_length - SPACER_length,
                                                          TSS - DISC_length - HEX10_length],
            ...
            for j in range(15, 21):
                ...
                for k in range(6, 11):
                    ...
                    if len(tempITR) < 20:
                        ...
                        dG_total, dG_apparent, dg10, dg35, dg_disc, dg_ITR, dg_ext10, dg_spacer, dg_UP = linear_free_energy_model(
                            tempUP, temp35, tempspacer, temp10, tempdisc, tempITR, self.dg10_0, self.dg10_3,
                            self.dg35_0, self.dg35_3, self.dmers, self.x10mers, self.spacers, self.model, self.inters)
                        dG_bind = dg10 + dg35 + dg_spacer + dg_ext10 + dg_UP
                        # dG_bind  = dg10 + dg_ext10 + dg_spacer + dg_UP
                        TSS_distance = i + len(tempUP) + len(temp35) + len(tempspacer) + len(temp10) + len(tempdisc)
                        # seq_query[(float(dG_bind), float(dG_total), TSS_distance)] = ((tempUP, temp35, tempspacer, temp10, tempdisc, tempITR),(dg10, dg35, dg_disc, dg_ITR, dg_ext10, dg_spacer, dg_UP))
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Process sequences with Promoter_Calculator.")
    parser.add_argument('sequences', nargs='+', help='List of sequences to process')
    parser.add_argument('--tss_range', type=int, nargs=2, default=[0, 1000], help='TSS range to use for predictions')
...
if __name__ == "__main__":
    ...
    for idx, output in enumerate(results):
        ...
        for (TSS, result) in output['Forward_Predictions_per_TSS'].items():
            print(f"Fwd TSS: {TSS}. TX Rate: {result['Tx_rate']}. Calcs: {str(result)}")
        for (TSS, result) in output['Reverse_Predictions_per_TSS'].items():
            print(f"Rev TSS: {TSS}. TX Rate: {result['Tx_rate']}. Calcs: {str(result)}")
